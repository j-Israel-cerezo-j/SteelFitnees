...
function requestCommentsByBranche(id) {
    ...
    request(resp => {
        var htmlComments = `<div style="padding: 15px;z-index:2;"> <b style="color:aquamarine;">Comentarios de nuestros clientes: </b>`;
        var ban = false;
        resp.forEach(item => {
            ban = true;
            htmlComments += `
                ...
                <p style="color:white;" class="card-text" style="margin-bottom: 0px;">                        
                    "${item.comment}"
                </p>
                <div class="linea"></div>`
        });
